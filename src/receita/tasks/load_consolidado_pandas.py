...
class LoadConsolidacaoPandas(Pipeline):
    ...
    def run(self) -> dict:
        ...
        self.log['duration_transform'] = (
            finish_pandas_transform -
            start_pandas_transform).total_seconds()
        self.load()
        return self.log
    ...
    def _venda_dataset(self) -> pd.DataFrame:
        ...
        return (
            pd.DataFrame(
                data=qs,
                columns=list(field_map.keys())+['valor_custo'])
    ...
    def _imposto_dataset(self) -> pd.DataFrame:
        ...
        return (
            pd.DataFrame(data=qs, columns=['loja_id', 'percent_imposto'])
            .astype(
                dtype={
                    'loja_id': 'int64',
                    'percent_imposto': 'float64'
                })
        )
